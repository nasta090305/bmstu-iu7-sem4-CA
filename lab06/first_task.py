# Функция решения системы нелинейных уравнений методом Гаусса
def solve_ls_by_gauss_method(matrix):
    n = len(matrix)
...
def simpson_integrate(a, b, function, x1, n) -> float:
    ...
    summ += 2 * sum([function(x1, steps[2 * j]) for j in range(1, N // 2)])
    summ += 4 * sum([function(x1, steps[2 * j - 1]) for j in range(1, N // 2 + 1)])
    summ += function(x1, steps[0])
    summ += function(x1, steps[N])

    return h / 3 * summ


# Функция, возвращающая значение полинома Лежандра степени n в точке value
def Pn(n, value):
    if n == 0:
        return 1
    elif n == 1:
...
# Функция поиска корней полинома Лежандра
...
# Функция второго интегрирования методом Гаусса (как первый интеграл используется метод Симпсона)
# nx - кол-во узлов по х, ny - кол-во узлов по у
# По х - Гаусс
# По у - Симпсон
def integral_by_gauss(ax, bx, nx, ny, function, start_function, end_function, epsilon):
    ...
    A = solve_ls_by_gauss_method(matrix)
