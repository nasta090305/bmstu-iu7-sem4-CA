...
from first_task import integral_by_gauss, solve_ls_by_gauss_method
...
if task == 1:
    ...
    with open("./data.txt", "r") as file:
        ...
        result_coef = solve_ls_by_gauss_method(matrix)
        ...
        x = [i for i in range(1, max_dot_count + 1)]

        plt.plot(x, result_x, label="Изменение количества узлов по направлению x")
        plt.plot(x, result_y, label="Изменение количества узлов по направлению y")
        plt.legend()
        plt.show()
elif task == 2:
    step_1 = 1
    ...
    buffer_l.insert(0, one_side_left_dif_derivative(step_1, ys[0], None))

    for i in range(1, len(xs) - 1):
        buffer_c.append(center_dif_derivative(step_1, ys[i + 1], ys[i - 1]))

    buffer_c.insert(0, center_dif_derivative(0, None, None))
    ...
    buffer_second.insert(0, center_dif_derivative(0, None, None))
